...
impl HomFunc{
    ...
    pub fn cast_before_HomFunc(&self, coefficients : Vec<u32>, inputs : &Vec<Ciphertext>, server_key : &ServerKey) -> Vec<Ciphertext>{
        // input encoded under a canonical encoding of form {0}, {1}
    ...
    pub fn cast_before_HomFunc_from_1(&self, inputs : Vec<Ciphertext>, server_key : &ServerKey) -> Vec<Ciphertext>{
        let coefficients : Vec<u32>= self.encodings_in.iter().map(|e| e.get_mono_encoding(true)).collect();
        self.cast_before_HomFunc(coefficients, &inputs, server_key)
    }
    ...
    pub fn modulus_switching(&self, inputs : Vec<Ciphertext>, p_in_vec : Vec<u32>, p_out : u32, server_key : &ServerKey) -> Vec<Ciphertext> {
        ...
        inputs.iter().zip(p_in_vec).map(|(x, p_i)| {
            if p_i != p_out {
                let HomFunc = HomFunc::new_canonical(vec![1], 1, p_i, p_out, 1, &|x| {x[0]});
                HomFunc.exec(&vec![x.clone()], &server_key)
            } else {
            x.clone()
            }
        }).collect()
