...
use tfhe::gadget::{prelude::*, ciphertext::Encoding};
...
impl StateSHA{
    ...
    pub fn encrypt_from_string(s : &String, client_key : &ClientKey, encoding_in : &Encoding) -> Self
    {
        ...
        for i in 0..5{
            ...
            for j in 0..5{
                data[i].push(Vec::new());
                for k in 0..64{
                    data[i][j].push(client_key.encrypt(bits[(i * 5 + j) * 64 + k], encoding_in));
                }
            }
        }
        Self {data}
    ...
    pub fn decrypt(&self, client_key : &ClientKey, encoding_out : &Encoding) -> String{
        let mut bits : Vec<bool> = Vec::new();
        for i in 0..5{
            for j in 0..5{
                for k in 0..64{
                    bits.push(client_key.decrypt(&self.data[i][j][k], encoding_out));
                }
            }
        }
...
fn cast_before_khi(state : StateSHA, HomFunc_cast : &HomFunc, server_key : &ServerKey) -> StateSHA{
    let mut new_vectors : Vec<Vec<Vec<Ciphertext>>> = Vec::new();
    for i in 0..5{
        new_vectors.push(Vec::new());
        for j in 0..5{
            ...
            for k in 0..64{
                new_vectors[i][j].push(HomFunc_cast.exec(&vec![state.get(i, j, k)], server_key));
...
fn khi(state : StateSHA, HomFunc_khi : &HomFunc, server_key : &ServerKey) -> StateSHA{
    let mut new_vectors : Vec<Vec<Vec<Ciphertext>>> = Vec::new();
    for i in 0..5{
        new_vectors.push(Vec::new());
        for j in 0..5{
            new_vectors[i].push(Vec::new());
            for k in 0..64{
                new_vectors[i][j].push(HomFunc_khi.exec(&vec![state.get(i, j, k), state.get(i, j+1, k), state.get(i, j+2, k)], server_key));
            }
        }
    }
    StateSHA {data : new_vectors}
...
fn sha_3(input : StateSHA, server_key : &ServerKey) -> StateSHA{
    let HomFunc_khi_and = HomFunc::new(
        vec![Encoding::new(HashSet::from([2]), HashSet::from([1]), 3) ; 2],
        Encoding::new(HashSet::from([1, 0]), HashSet::from([2]), 3),
        Encoding::new_canonical(1, 2),
        2,
    ...
    let HomFunc_cast_before_khi = HomFunc::new(
        vec![Encoding::new_canonical(1, 2)],
        Encoding::new_canonical(1, 2),
        Encoding::new(HashSet::from([2]), HashSet::from([1]), 3),
        1,
        &|x : Vec<bool>| -> bool{   x[0]    }
    );
    ...
    let state_before_khi = cast_before_khi(state_pi, &HomFunc_cast_before_khi, &server_key);
    let state_khi = khi(state_before_khi, &HomFunc_khi_and, server_key);
...
pub fn demo_sha3() {
    // a run of sha3
    let (client_key, server_key) = gen_keys(&SHA3_PARAMETERS);
    let encoding_xor = Encoding::new_canonical(1, 2);
